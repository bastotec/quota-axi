...
- The `models` command resolves the Claude credential once and shares that resolution between its quota read and its live model listing, so absent an invalidation one run performs one macOS Keychain value read and both halves answer from the same account. A delegated refresh invalidates the shared resolution, so the listing reads back the session the vendor CLI wrote. A definitive rejection invalidates it too, which covers a store rotated outside this command; it does not stop the rejected credential itself from being resolved again and presented to the listing endpoint, and on macOS that second resolution is a second Keychain value read.
