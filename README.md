...
| `windows[].percentUsed`, `windows[].startsAt`, `windows[].windowSeconds`, `windows[].modelScope`                                      |
...
`auth_required` says a credential is needed, never that the account is signed out, and `state.reason` separates three states that lead to three different actions. `no_local_credential` means no store quota-axi reads held a credential to test: that is a fact about where quota-axi looked, not evidence about the account, which may be answering live quota to whoever holds its credential elsewhere - look elsewhere. `local_credential_unusable` means a store was read and yielded nothing the adapter can send - a malformed Codex `auth.json`, one holding no ChatGPT login, an unparseable opencode `auth.json`, a Z.AI entry whose key is unusable - so no endpoint ever examined a credential: check what that store holds. It does not claim a credential was there and broken; that is more than the run established. A store quota-axi could not open at all (permissions, a directory in its place) is not in that bucket either and is not an auth problem: it reports the read failure and leaves the contents question open, because the file may hold a perfectly good login. No reason at all means a credential was found and a first-party endpoint refused it - the only evidence that earns a sign-out, and the only one that calls for a new sign-in. Read them apart in TOON as well: the `attention[]` detail carries `reason no_local_credential` or `reason local_credential_unusable`. `--tui` renders them as `no credential` and `no usable credential` rather than `signed out`, and counts those providers separately in its header instead of as signed out. Codex and Z.AI publish the reasons today; the remaining adapters still report an empty-handed read as a sign-out and are being routed through the same helper.
...
A model-specific `scope` is the vendor-spelled scope identity the provider's adapter carried on the window (`windows[].modelScope.id`), shared by every period and repeat of that scope, so it is not always a `model:`-prefixed id - Anthropic's Opus week reports `seven_day_opus`. See [Model identity comes from the vendor](#model-identity-comes-from-the-vendor).
...
| Codex                  | Identifies exact 18,000-second and 604,800-second periods as `five_hour` and `weekly`, regardless of source slot; periods without a duration retain their positional identity. Additional model- or feature-scoped limits use `model:<id>:5h` / `model:<id>:7d`, and code-review limits use `code_review_five_hour` / `code_review_weekly`. A metered limit id such as `codex_bengalfox` is an opaque bucket name, so a named limit's `modelScope` asserts a `modelId` only when the vendor sent its own `normal_model_slug` / `normalModelSlug` for that bucket, and otherwise carries only the vendor's `limit_name`. Unfamiliar durations remain honest `<hours>h` windows instead of being classified as known periods. Duplicate derived IDs are preserved with `_2`, `_3`, and later suffixes, and the repeat shares the scope it repeats. Optional credit balance data can also appear.                                                               |
...
Model identity is the provider's, never quota-axi's. On each run the join reads the vendor's own model listing and reports the models that listing named, and it attributes a model-scoped quota window by matching what the vendor said about that window's scope against the vendor's own model identity.

The scope is carried as typed data from the adapter that read it, never recovered from the window id. Window ids are provider grammar and the grammars conflict - an underscore is part of a slugified Claude display name but a duplicate counter in a Codex id, and a trailing `:N` is a Codex period but an Alibaba repeat - so nothing downstream takes one apart. Each `kind: "model"` window instead carries a `modelScope` with the scope's vendor-spelled identity (`id`), the vendor's own model id when it gave one (`modelId`), and the vendor's own name for the scope (`name`). A window whose id spells a period or a repeat keeps that in its id alone; every period and repeat of one scope shares that scope's identity. `effectiveAvailability[].scope` reports that scope identity, so a window whose id is not `model:`-prefixed at all - Anthropic's `seven_day_opus` - is attributed like any other. A model window whose adapter carried no scope is not attributed at all: the join declines to guess one back out of the window id and names the window in `unmatchedWindowIds`, so the account-wide bound never stands in for it.

Attribution uses only the vendor's words. When the vendor named a model id, the scope points at one model and is attributed only on exact vendor identity - that id or the model's display name, or that identity plus a dated vendor snapshot suffix - so a window scoped to `Claude Opus 4` never lands on `claude-opus-4-5-20251101`. When the vendor named no id, all quota-axi has is the vendor's name for the scope: a name carrying a version component names that version and is matched exactly, and a name carrying none names a family (Anthropic sends `scope.model.id: null` with `display_name: "Fable"`, and meters an Opus-scoped week), so it is attributed to every live model of that family and to nothing else. A vendor can meter one model under more than one scope - a family bound and a model-specific bound both cover it - and the order the vendor happened to list them in decides nothing: the row reports the most binding matching scope, so it never publishes headroom a tighter bound denies. quota-axi does not follow the listing's pages; if the vendor says the page it read is not the whole lineup, that lineup is disclosed as `partial_lineup`. Claude reads `GET https://api.anthropic.com/v1/models`, a read-only listing that starts no session and spends none of the quota being measured. A provider gets a live catalog source only once such a first-party, no-spend listing endpoint is established empirically; Claude has one, and Codex, Grok, and Kimi have none yet. `no_live_catalog_source` means no live catalog reading was available for this call - either because the provider has no listing endpoint established (Codex, Grok, Kimi on every run today) or because the caller of the join supplied no reading, which a library consumer calling `createModelsResponse` without `liveCatalogs` does even for Claude.
...
| `catalogSources[].reason`                    | Why a live lineup is missing (`no_live_catalog_source`, `no_credential`, `credential_rejected_this_run`, `catalog_http_<status>`, `catalog_timeout`, `catalog_unreachable`, `catalog_unrecognized`), or `partial_lineup` when a `live` lineup is the vendor's words but not all of them |
...
| `unverifiedAttributions[]`                   | A model window a built-in entry claims for a model (`provider`, `windowId`, `modelId`) with no live lineup confirming the two names mean the same thing                                                                                                                                 |

When a provider's live catalog cannot be read, quota-axi falls back to its built-in lineup but never presents it as the vendor's answer: those rows are marked `unverified_builtin` (`identity=unverified` in TOON) and `catalogSources[].reason` says why. Such a row still carries the model-window attribution its built-in `windowIds` entry declares - Codex, Grok, and Kimi have no live listing endpoint, so that is the only attribution available for them - but the claim is disclosed rather than presented as vendor-confirmed: every window attributed that way is named in `unverifiedAttributions`, in both TOON and `--json`. A built-in attribution the vendor's own live lineup disproves is deleted from the catalog instead; one that is merely unverified is kept and disclosed. `model:codex_bengalfox` -> `gpt-5.3-codex` is the remaining unverified one: OpenAI exposes no supported model-to-bucket mapping, its own client treats that limit id as opaque, and the entry's `notes` in `src/model-kb.ts` records the evidence. Treat such rows as possibly out of date, and `unmatchedWindowIds` as a model window the lineup in use did not account for at all.
...
That table is the whole search. quota-axi reads each vendor's own credential store and the declared agent-harness stores (Pi, opencode) that own one, and it does not go looking for credentials that other tools have enrolled in their own state directories - a proxy, a gateway, or any wrapper that holds its own copy. Two reasons, and neither is squeamishness: those layouts belong to software this tree does not ship or review, so their shape and lifecycle could change under a credential read at any time; and such a tool commonly enrols several accounts for one provider, where picking one would publish a number for an account the caller may not be using, which is precisely the kind of unattributed reading [Model identity comes from the vendor](#model-identity-comes-from-the-vendor) exists to prevent. The cost of that boundary is borne honestly rather than silently: a provider whose credential lives somewhere quota-axi does not read reports `no_local_credential` (see [Provider `state`](#provider-state)), which says quota-axi found none rather than that the account is signed out.
...
- The `models` command resolves the Claude credential once and shares that resolution between its quota read and its live model listing, so one run performs one macOS Keychain value read and both halves answer from the same account. A delegated refresh that actually started drops that shared resolution, so the listing reads back the session the vendor CLI wrote; an outcome where the vendor CLI never ran keeps the shared resolution, because the store provably was not rewritten.
- Within one run, a credential Anthropic definitively rejected is not presented again. The quota read's HTTP 401 withholds that credential from the model listing, which reports `credential_rejected_this_run` without contacting the listing endpoint; a retry after a delegated refresh that rotated nothing likewise withholds it rather than collecting a second identical 401, and still reports Anthropic's own sign-in verdict rather than a missing credential. Withholding is recorded per source in `--full` attempts as `skipped` with that same reason. The latch is on the credential, not on the source or the provider: a sibling store holding the same rejected token is withheld too - even later in the same pass, so Anthropic receives one 401 rather than two - while a sibling store the vendor never rejected still answers, and a token the vendor CLI wrote in the rejected one's place inherits nothing from it. Only HTTP 401 latches - HTTP 403, timeouts, and server failures leave the credential available, because a WAF denial can use 403 for a valid bearer. It is per invocation and in memory: nothing about a rejection is written anywhere or carried into the next run, and what the latch holds is a digest, never a credential value.
- The live model listing selects credentials in the same order as the quota read and never refreshes one. Only HTTP 401 moves to the next stored credential; every other outcome, HTTP 403 included, stops there and reports the listing as `unavailable` with that reason, so one account's lineup can never be published beside another account's windows.
...
- The bearer probe asks one endpoint, `https://chatgpt.com/backend-api/wham/usage`.
  `backend-api/codex/usage` was a second candidate and is not one any more: it answers a Cloudflare bot-management challenge (403, `cf-mitigated: challenge`, HTML) identically for a valid bearer, another account's bearer, and no credential at all, so it never examines a credential and can never return a reading.
  Keeping it cost every Codex failure a round-trip to it, and because this probe reads 403 as a rejection, the only thing it could contribute was a WAF challenge in the shape of the user's sign-in verdict.
...
| Cached reports                         | Only fresh provider snapshots with windows are cached. A model window's `modelScope` travels with it, and snapshots written before scopes were carried are no longer reused at all: their schema version is rejected, so no stale reading can attribute a scope-less model window to the account-wide bound instead.                                                                  |
