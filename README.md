...
When a provider's live catalog cannot be read, quota-axi falls back to its built-in lineup but never presents it as the vendor's answer: those rows are marked `unverified_builtin` (`identity=unverified` in TOON) and `catalogSources[].reason` says why. A built-in row carries no model-window attribution: a model-scoped window quota-axi cannot attribute to a vendor-named model is reported unattributed in `unmatchedWindowIds` rather than pinned to a guessed model. Treat such rows as possibly out of date, and `unmatchedWindowIds` as a model window the lineup in use did not account for.
