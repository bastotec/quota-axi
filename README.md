...
| Cached reports                         | Only fresh provider snapshots with windows are cached. A model window's `modelScope` travels with it; a snapshot written before scopes were carried simply has none, and a stale reading then attributes no model window rather than recovering a scope from the window id.                                                                                                           |
