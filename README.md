...
| `catalogSources[].reason`                    | Why a live lineup is missing (`no_live_catalog_source`, `no_credential`, `catalog_http_<status>`, `catalog_timeout`, `catalog_unreachable`, `catalog_unrecognized`), or `partial_lineup` when a `live` lineup is the vendor's words but not all of them |
...
- On macOS the `models` command reads the Claude credential twice in one run: once for quota and once for the live model listing. If you answer the Keychain prompt with plain "Allow", macOS prompts again inside that same command; choosing "Always Allow" avoids the second prompt.
